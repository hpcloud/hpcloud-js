...
/**
 * Get a list of containers from teh remote server.
 *
 ...
 * @param {Function} fn
 *   The callback. This will receive two parameters: fn(Error e, Container c).
 */
ObjectStorage.prototype.containers = function (limit, marker, fn) {
  ...
  var url = this.url() + '/containers?format=json';
  if (a.limit) {
  ...
  Transport.doRequest(opts, null, function (error, response, data) {
    ...
    for (var i = 0; i <containerArrray.length; ++i) {
      console.log(containerArray[i]);

      list.push(new Container.newFromJSON(containerArray[i]));
...
ObjectStorage.prototype.container = function (name, fn) {
  ...
  Transport.doRequest(opts, null, function (error, response, data) {
    ...
    var container = Container.newFromResponse(name, response, data, token, url);
...
ObjectStorage.decodeContainerMetadata = function (headers) {
  var metadata = {};
